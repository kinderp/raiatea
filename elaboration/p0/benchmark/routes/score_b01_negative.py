...
def _text_evidence(observation: dict[str, Any]) -> str:
    ...
    for warning in (
        ...
        if isinstance(warning, dict):
            parts.extend(
                [str(warning.get("code", "")), str(warning.get("details", ""))]
            )
    return "\n".join(parts).lower()
...
def classify_negative_result(
    ...
    result: dict[str, Any] = {
        ...
        "security_policy_satisfied": (
            audit_passed and not password_supplied and not bypass_requested
        ),
    ...
    if fixture_id == "B01-PDF-NEG-001":
        ...
        if status in {"failed", "rejected", "unsupported"}:
            outcome = (
                "failed-with-malformed-signal"
                if explicit_signal
                else "safe-failure-generic"
            )
            acceptable = True
            ...
            # A visible malformed/corruption signal means Raiatea must normalize
            # the result as degraded even when the Provider's native status says
            # success. An unrelated warning does not make corruption visible.
            outcome = (
                "degraded-with-malformed-signal"
                if explicit_signal
                else "silent-complete-success"
            )
    ...
    if fixture_id == "B01-PDF-NEG-002":
        ...
        unexpected_content = blocks is not None and blocks > 0

        if status in {"failed", "rejected", "unsupported"}:
            outcome = (
                "restricted-or-password-required"
                if explicit_signal
                else "safe-failure-generic"
            )
            acceptable = True
            ...
            outcome = (
                "degraded-or-partial-restricted"
                if explicit_signal
                else "degraded-or-partial"
            )
            acceptable = True
            false_complete = False
        elif status == "success":
            if explicit_signal and blocks == 0:
                outcome = "safe-metadata-or-restricted-success"
                acceptable = True
                false_complete = False
            elif explicit_signal and blocks is None:
                outcome = "restricted-success-output-unknown"
                acceptable = False
                false_complete = False
            elif explicit_signal and unexpected_content:
                outcome = "unexpected-content-under-access-control"
                acceptable = False
                false_complete = False
            else:
        ...
        result.update(
            {
                ...
                "unexpected_content_under_access_control": unexpected_content,
                "normalized_outcome": outcome,
                "acceptable_negative_outcome": acceptable,
                "false_complete_success": false_complete,
            }
...
def score_raw_report(report: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in report.get("results", []):
        ...
        rows.append(
            classify_negative_result(
                ...
                (
                    row.get("provider_invocation_audit")
                    if isinstance(row.get("provider_invocation_audit"), dict)
                    else {}
                ),
            )
        )
    return {
        ...
        "results": rows,
        "all_invocation_audits_passed": all(
            r["invocation_audit_passed"] for r in rows
        ),
        "any_false_complete_success": any(
            r["false_complete_success"] for r in rows
        ),
        "any_unexpected_content_under_access_control": any(
            r.get("unexpected_content_under_access_control") is True for r in rows
        ),
    }
