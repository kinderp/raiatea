#!/usr/bin/env python3
"""Official PDF1b Poppler ExtractorPlugin.

The plugin receives one Core-private PDF copy via opaque AssetHandle, verifies the
accepted Poppler reference binaries, and emits only path-free provider-native
PopplerObservation evidence. Raiatea Core owns E-05 normalization/persistence.
"""
...
_RUNTIME_SPEC = importlib.util.spec_from_file_location("pdf1b_poppler_runtime", RUNTIME_VALIDATOR_PATH)
...
def _handshake(manifest: dict[str, Any], runtime_instance_id: str) -> dict[str, Any]:
    return {
        ...
        "advertised_profiles": [
            {"capability_id": "extract.run", "profile_id": "pdf-poppler-pdftohtml-xml"}
        ],
        "observed_at": _now(),
    }
...
def _output_target(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(targets, list) or len(targets) != 1 or not isinstance(targets[0], dict):
        raise ValueError("exactly-one-poppler-observation-output-target-required")
    target = targets[0]
    if target.get("media_type") != POPPLER_OBSERVATION_MEDIA_TYPE:
        raise ValueError("poppler-observation-media-type-required")
    return target
...
def _diagnostic(
    ...
    return {
        ...
        "diagnostic_id": f"diag:{runtime_instance_id}:{invocation_id}:poppler-observed",
        "runtime_instance_id": runtime_instance_id,
        "invocation_id": invocation_id,
        "severity": "info" if status == "success" else "warning",
        "code": "pdf-poppler-provider-observation-created",
        "message": f"Poppler pdftohtml status={status} blocks={blocks} figures={figures}",
        "observed_at": _now(),
    }
...
def _invoke(
    ...
    if request.get("capability") != {
        ...
        "profile_id": "pdf-poppler-pdftohtml-xml",
    }:
        return (
            _failed(
                manifest,
                ...
                message="PopplerPdfExtractorPlugin supports only extract.run/pdf-poppler-pdftohtml-xml",
            ),
            [],
        )
    rights_ref = request.get("runtime_context", {}).get("rights_decision_ref")
    if not isinstance(rights_ref, str) or not rights_ref.startswith("rights-decision:"):
        return (
            _failed(
                ...
                message="Core PDF extraction rights_decision_ref is required",
            ),
            [],
        )
    try:
        ...
        bundle = run_poppler_pdf(
            source_bytes,
            source_ref_id=source_ref_id,
            source_fingerprint=fingerprint,
        )
        verify_reference_poppler(bundle["provider"])
        payload = encode_poppler_observation_bundle(bundle)
        ...
        return (
            _failed(
                ...
                request,
                started_at=started_at,
                code="pdf-poppler-extraction-failed",
...
def main() -> int:
    manifest = _manifest()
    runtime_instance_id = f"runtime:pdf1b:poppler:{os.getpid()}"
