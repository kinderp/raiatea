...
- [`evaluator-session-batch-manifest.md`](evaluator-session-batch-manifest.md) — closed deterministic inventory of explicitly supplied evaluator-session v1 files, canonical paths, SHA-256 digests, ordering and duplicate rejection without reading record contents.
- [`batch-record-validation.md`](batch-record-validation.md) — local contextual intake that resolves only manifest-listed files below one root, verifies regular-file safety and SHA-256, validates evaluator-session v1 values, rejects byte/semantic duplicates and returns immutable snapshots without aggregation.
- [`evaluator-session-aggregate-summary.md`](evaluator-session-aggregate-summary.md) — deterministic local synthesis of validated snapshots into provenance digests, platform/launcher counts, canonical result totals and explicitly assigned approved theme labels, without copying observations or learner data.
- [`field-pilot-synthesis-kit.md`](field-pilot-synthesis-kit.md) — final reproducible kit composition, complete local example, evaluator instructions, cleanup, checksum coverage and cross-platform acceptance boundary.
...
16. Read the evaluator-session batch-manifest contract before adding multi-record intake, duplicate detection or synthesis behavior.
17. Read the batch-record validation contract before changing root resolution, digest verification, contextual record checks or duplicate semantics.
18. Read the evaluator-session aggregate contract before changing summary fields, approved theme labels, provenance or count semantics.
19. Read the synthesis-kit contract before changing packaged tools, the canonical example, cleanup instructions or cross-platform acceptance.
20. Read current schemas, validators, examples, tests, and the prototype README before implementing a micro-step.

## Current implementation boundary

Canonical pedagogical modules and learner-evidence v1/v2 contracts remain unchanged. The completed pilot provides a deterministic offline two-module journey, read-only route dashboard, module-scoped v1 export, evaluator guidance, and full browser acceptance. The evaluator release remains reproducible and independently verifiable; the final synthesis kit composes the cross-platform field-pilot package with importable session, manifest, intake and aggregate tools, a deterministic complete example, explicit cleanup guidance, regenerated checksums and independent archive verification. Linux and Windows exercise the packaged workflow in CI; macOS remains documented as POSIX manual parity. Accounts, analytics, LMS integration, signing, installers, registries, deployments, services, remote submission and automatic updates remain deferred.
