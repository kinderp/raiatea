...
Optional media type, length, content fingerprint and expiry support validation. The later runtime implementation must bind a handle to an authorized scope/lease and reject expired or over-scope access.
...
- terminal invocation status is `cancelled` only with cancelled runtime error evidence;
...
Diagnostics are structured evidence separate from lifecycle and domain outcome. Contract fields never carry a secret value. A runtime implementation that knows injected secret values must redact/reject diagnostic messages containing those values before persistence or display.
