#!/usr/bin/env python3
"""Run B01-PDF-006 through the already-pinned B-01 reference routes.

This helper is deliberately evidence-first. It constrains the accepted Poppler,
Tika and Docling B-01 runners to the authored formula fixture and preserves raw
and normalized observations for inspection. It does not infer mathematical
structure from typography/position and does not define E-05.
"""
...
FORMULA_FIXTURES = ["B01-PDF-006"]


def _limit_to_formula_fixture(module: Any) -> None:
    """Constrain an existing reference runner without changing its defaults."""
    module.B01_NORMAL_FIXTURES = list(FORMULA_FIXTURES)


def run_poppler(output: Path, evidence_source_commit: str | None) -> dict[str, Any]:
    import measure_b01

    _limit_to_formula_fixture(measure_b01)
...
def run_tika(
    ...
    _limit_to_formula_fixture(measure_tika_b01)
...
def run_docling(
    ...
    _limit_to_formula_fixture(measure_docling_b01)
