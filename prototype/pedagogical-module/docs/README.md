...
19. Read current schemas, validators, examples, tests, and the prototype README before implementing a micro-step.

## Current implementation boundary

Canonical pedagogical modules and learner-evidence v1/v2 contracts remain unchanged. The completed pilot provides a deterministic offline two-module journey, read-only route dashboard, module-scoped v1 export, evaluator guidance, and full browser acceptance. The evaluator release remains reproducible and independently verifiable; the field-pilot archive composes checksummed POSIX and PowerShell launch/stop helpers, local instructions, a closed evaluator-session tool, canonical fixture and acceptance matrix. The evaluator record is optional, non-identifying, explicit-only and never read by the pilot. The batch manifest inventories explicitly supplied local records; contextual intake resolves only those paths under one supplied root, verifies regular files and SHA-256, validates evaluator-session v1 content and rejects duplicate bytes or semantic values. A closed aggregate now derives only deterministic counts, approved explicit theme labels and digest provenance from immutable validated snapshots. Packaging remains deferred to #91. Linux and Windows have executable CI coverage; macOS is documented as manual POSIX parity. Accounts, analytics, LMS integration, signing, installers, registries, deployments, services, remote submission and automatic updates remain deferred.
