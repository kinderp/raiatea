...
The contextual checker receives one structurally valid source module, one structurally valid target module, and one structurally valid migration manifest. It verifies exact module IDs, opaque revision identities, and the complete ordered stable-step inventory declared for both endpoints.

```bash
python prototype/pedagogical-module/build/check_evidence_migration_manifest_context.py \
  prototype/pedagogical-module/tests/fixtures/evidence-migration-context/exact-source.json \
  prototype/pedagogical-module/tests/fixtures/evidence-migration-context/exact-target.json \
  prototype/pedagogical-module/tests/fixtures/evidence-migration-context/exact-manifest.json
```

All three inputs are structurally validated before contextual comparison. Structural failures are reported under `sourceModule`, `targetModule`, and `manifest` namespaces in fixed order. Contextual mismatches are reported under the manifest endpoint that makes the false declaration. The checker is side-effect free, never infers revision ordering, and does not classify learner evidence, select a migration path, preview a transition, or apply state changes.
...
The architecture decision for durable module identity, explicit revisions, immutable step IDs, and authored migration responsibilities is documented in [`docs/module-evolution-and-evidence-compatibility.md`](docs/module-evolution-and-evidence-compatibility.md). The executable authoring rules are in [`docs/module-revision-authoring.md`](docs/module-revision-authoring.md), the v2 stable-identity contract is in [`docs/learner-evidence-v2-stable-identity.md`](docs/learner-evidence-v2-stable-identity.md), the exact contextual contract is in [`docs/learner-evidence-v2-exact-compatibility.md`](docs/learner-evidence-v2-exact-compatibility.md), the migration-manifest contract is in [`docs/learner-evidence-migration-manifest.md`](docs/learner-evidence-migration-manifest.md), the contextual manifest-validation contract is in [`docs/learner-evidence-migration-manifest-contextual-validation.md`](docs/learner-evidence-migration-manifest-contextual-validation.md), and the documentation index is available in [`docs/README.md`](docs/README.md).
...
- contextual manifest validation compares one manifest with exact supplied source and target revisions and ordered stable-step inventories;
- browser v2 export/import, publication-history lookup, compatibility classes B–E, migration preview, and migration application are not implemented;
...
Publication registries, compatibility classification, migration previews, and state-changing migration remain separate reviewable increments.
...
├── docs/learner-evidence-migration-manifest-contextual-validation.md
├── src/template.html
├── src/module.css
├── src/module.js
...
├── build/check_evidence_migration_manifest_context.py
...
├── tests/fixtures/evidence-migration-context/
...
├── tests/test_evidence_migration_manifest_context.py
...
- migration manifests have separate structural and exact contextual validators; neither classifies evidence, selects paths, previews, or applies transitions;
...
1. Add compatibility classification and human-readable migration preview without state changes.
2. Add explicitly confirmed migration while preserving the original evidence copy.
3. Add explicit browser v2 export/import only after contextual compatibility and migration policy are complete.
