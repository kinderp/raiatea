...
def _write_summary(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- model artifact files: `{model.get('file_count')}`, bytes `{model.get('bytes')}`",
        f"- model artifact manifest SHA-256: `{model.get('manifest_sha256')}`",
    ...
    for fixture_id in ["B01-PDF-001", "B01-PDF-002"]:
        ...
        lines.extend(
            [
                ...
                f"- source coordinates: `{coords.get('status')}`; contained `{coords.get('contained_count')}/{coords.get('expected_count')}` when measured",
...
def run_baseline(
    ...
    for fixture_id in ["B01-PDF-001", "B01-PDF-002"]:
        ...
        result["model_manifest_sha256"] = observation.get(
            "model_artifacts", {}
        ).get("manifest_sha256")
    ...
    report = {
        ...
        "measurement_limits": [
            "The route uses only the prefetched model artifact root recorded in this evidence step.",
