...
- **F9 — Aggregate publication failed on Windows because the `tempfile.mkstemp` descriptor remained open while the staging path was reopened.** Resolved by closing the descriptor immediately, retaining no-replace hard-link publication, adding a descriptor-lifecycle regression and exercising the packaged CLI plus canonical aggregate comparison in Windows CI.
