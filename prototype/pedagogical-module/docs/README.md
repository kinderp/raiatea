...
- [`learner-evidence-v2-confirmed-migration-application.md`](learner-evidence-v2-confirmed-migration-application.md) — explicit confirmation, validated-candidate applicability, atomic create-if-absent destination, source-byte preservation, post-write verification, and rollback boundaries.
...
10. Read the confirmed-application contract before creating a destination evidence copy, requesting confirmation, or adding any operation that can write learner-evidence v2.
11. Read the current schemas, validators, examples, tests, and prototype README before implementing a micro-step.

## Current implementation boundary

Canonical pedagogical modules require a positive integer `revision` and one unique lowercase/digit/hyphen `id` for every pedagogical step. Learner-evidence v1 remains frozen under its exact module-ID, ordered-index, and authored-step-title compatibility rules. Learner-evidence v2 has a separate closed schema, example, and side-effect-free structural validator carrying exact revision identity and stable step IDs. A separate exact contextual checker compares a structurally valid v2 document with one supplied canonical module revision. A closed migration-manifest schema and standalone validator describe complete same-module source/target inventories and `preserve`, `retire`, and `introduce` mappings. A side-effect-free contextual checker binds a manifest to exact supplied source and target revisions. The preview engine classifies exact, declared-lossless, declared-partial, incompatible, and unsupported direct transitions and may construct a non-persisted candidate only when current position is deterministic. The confirmed application layer accepts only declared-lossless or declared-partial previews with an available validated candidate, requires the exact `APPLY-NEW-COPY` token, atomically creates one absent destination, verifies the installed v2 document against the exact target, and proves the source evidence bytes remain unchanged. Publication registries, manifest path selection/chaining, split/merge semantics, browser v2 export/import or confirmation UI, retired-current position selection, cloud transfer, and any change to learner-evidence v1 remain separate increments.
