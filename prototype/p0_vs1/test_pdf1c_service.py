...
def make_provider_bundle(source_ref_id: str, fingerprint: str, status: str) -> dict:
    blocks = []
...
class FakePluginIO:
    ...
    def read_completed_output(self, output_target, completed):
        hook = type(self).completion_hook
        if hook is not None:
            hook()
        return bytes(type(self).output_bytes)
...
class FakeClient:
    ...
    def invoke(self, request):
        hook = type(self).invocation_hook
        if hook is not None:
            hook(request)
        ...
        completed = {
            ...
            "byte_length": len(FakePluginIO.output_bytes),
            "fingerprint": "sha256:" + hashlib.sha256(FakePluginIO.output_bytes).hexdigest(),
...
class Pdf1cServiceFixture(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.source_ref = self._source_ref_for_location("single.pdf")
        ...
        FakePluginIO.output_bytes = b""
        FakePluginIO.completion_hook = None
        FakeClient.invocation_hook = None
    ...
    def _source_ref_for_location(self, location: str) -> str:
    ...
    def _configure_provider_output(self, status: str) -> None:
        payload = self.store.load().payload
        source = next(
            row
            for row in payload["vs1c"]["source_references"]
        ...
        bundle = make_provider_bundle(self.source_ref, source["fingerprint"], status)
        FakePluginIO.output_bytes = canonical_json_bytes(bundle)

    def _run_with_fakes(self, *, status: str = "success") -> dict:
        self._configure_provider_output(status)
        with (
            patch(
                "prototype.p0_vs1.pdf1c_service.verify_reference_docling",
            ...
            patch(
                "prototype.p0_vs1.pdf1c_service.build_docling_extra_env",
                return_value={
...
class Pdf1cServiceTests(Pdf1cServiceFixture):
    def test_denied_rights_stop_before_provider_verification_and_source_read(self) -> None:
        with (
            ...
            patch.object(
                self.broker,
                "issue_read_handle",
                wraps=self.broker.issue_read_handle,
            ) as issue_handle,
        ):
    ...
    def test_success_publishes_independent_docling_current_e05_state(self) -> None:
        result = self._run_with_fakes(status="success")
        ...
        current = state["current_extractions"][0]
        representation = next(
        ...
        self.assertEqual(
            representation["units"][0]["semantic_role"]["value"],
    ...
    def test_degraded_is_attempt_evidence_not_current_content(self) -> None:
        result = self._run_with_fakes(status="degraded")
    ...
    def test_catalog_change_during_provider_run_rejects_stale_publication(self) -> None:
        ...
        def mutate_catalog(_request) -> None:
        ...
        FakeClient.invocation_hook = mutate_catalog
        with (
            patch(
                "prototype.p0_vs1.pdf1c_service.verify_reference_docling",
                return_value=provider(),
    ...
    def test_existing_poppler_state_is_not_modified_by_docling_publication(self) -> None:
        ...
        self.assertTrue(self._run_with_fakes(status="success")["published_current"])
    ...
    def test_persisted_provider_reference_tamper_is_rejected_before_new_run(self) -> None:
        self._run_with_fakes(status="success")
        current = self.store.load()
        payload = deepcopy(current.payload)
        payload["pdf1c"]["current_extractions"][0]["provider_observation"]["provider"][
            "model_payload_sha256"
        ] = "sha256:" + "0" * 64
        self.store.save(payload, expected_revision=current.revision)
        with self.assertRaises(docling_reference.DoclingReferenceError):
            self.service.extract(
                self.source_ref,
                rights_evidence_state="known-permitted",
            )
    ...
    def test_plugin_record_ref_claim_is_rejected(self) -> None:
        ...
        with (
            ...
            with self.assertRaisesRegex(
                DoclingPdfExtractionError,
                "must-not-claim-core-e05-record-refs",
            ):
                self.service.extract(
                    self.source_ref,
                    rights_evidence_state="known-permitted",
