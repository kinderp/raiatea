...
class PluginManifestTests(unittest.TestCase):
    ...
    def test_prerelease_plugin_version_is_supported(self):
        value = load_example("local-read-only-source.json")
        value["plugin"]["version"] = "0.1.0-rc.1"
    ...
    def test_free_form_entrypoint_arguments_fail(self):
        value = load_example("benchmark-backed-extractor.json")
        value["entrypoint"]["command"] = ["raiatea-benchmark-extractor", "--token=secret"]
        with self.assertRaisesRegex(VALIDATOR.ManifestError, "entrypoint-command-not-structural"):
            VALIDATOR.validate(value)

    def test_python_module_entrypoint_with_runtime_flags_fails(self):
        value = load_example("local-read-only-source.json")
        value["entrypoint"]["command"] = ["python", "-m", "raiatea_local_source", "--root", "/tmp"]
        with self.assertRaisesRegex(VALIDATOR.ManifestError, "entrypoint-command-not-structural"):
            VALIDATOR.validate(value)
